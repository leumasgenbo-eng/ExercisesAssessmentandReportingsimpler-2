...
const Topbar: React.FC<TopbarProps> = ({ 
  ...
  onPrint, onLogout, isFocusMode = false, isAdminUnlocked = false, isInstitutionalized = false,
  userRole = 'GUEST'
}) => {
  ...
  const getStageColor = (group: string) => {
    switch(group) {
      ...
      case 'KINDERGARTEN': return 'bg-amber-500 border-amber-500';
  ...
  const isFullAdmin = isAdminUnlocked || userRole === 'SCHOOL_ADMIN' || userRole === 'SUPER_ADMIN';
  if (isFullAdmin) {
    portalOptions.push({ id: 'FACILITATORS', label: 'Staff', icon: '👨‍🏫' });
    portalOptions.push({ id: 'ADMIN', label: 'Admin', icon: '⚙️' });
  }
      ...
      <div className="bg-sky-950 text-white shadow-xl">
        <div className="max-w-[1500px] mx-auto px-2 md:px-4 h-14 flex justify-between items-center gap-1">
          ...
          <div className="flex items-center gap-1 shrink-0">
             ...
             <button onClick={onLogout} className="bg-slate-800 hover:bg-rose-600 text-white p-2 rounded-lg transition-all group">
