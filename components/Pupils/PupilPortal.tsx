...
const PupilPortal: React.FC<Props> = ({ fullState, onUpdateState, onUpdateBookCounts, isFocusMode = false, setIsFocusMode, isIndividualOnly = false }) => {
  ...
  const tabs = isIndividualOnly 
    ? [
        { id: 'BROADSHEET', label: 'My Performance Chart', color: 'bg-indigo-600', hover: 'hover:text-indigo-400' },
        { id: 'REGISTRY', label: 'My Growth Record', color: 'bg-indigo-600', hover: 'hover:text-indigo-400' },
      ]
    : [
  ...
  return (
    <div className={`space-y-8 animate-in transition-all duration-700 ${isFocusMode ? 'p-0 pt-8' : ''}`}>
      <div className={`no-print flex flex-col md:flex-row items-center justify-center gap-6 ${isFocusMode ? 'mb-12' : ''}`}>
        {isIndividualOnly && (
           <div className="text-center mb-4">
              <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tighter">Academic Performance Summary</h3>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Individual Progress Intelligence</p>
           </div>
        )}
        <div className="bg-white p-2 rounded-[2.5rem] border-2 border-slate-100 shadow-2xl flex flex-wrap justify-center gap-2">
          {tabs.map((tab) => (
            <button 
              key={tab.id}
              onClick={() => setActiveTab(tab.id as PupilSubView)}
              className={`px-8 py-4 rounded-[1.8rem] text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab.id ? `${tab.color} text-white shadow-xl scale-105` : `text-slate-400 ${tab.hover}`}`}
            >
              {tab.label}
            </button>
          ))}
        ...
        {!isIndividualOnly && (
          <button 
            ...
            className={`p-4 rounded-2xl border-2 transition-all shadow-xl ${isFocusMode ? 'bg-amber-500 border-amber-600 text-white rotate-180' : 'bg-white border-slate-200 text-slate-400'}`}
          >
            {isFocusMode ? (
              ...
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4" /></svg>
            )}
          </button>
        )}
      </div>
